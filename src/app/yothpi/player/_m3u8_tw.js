  ...
  return new Promise((resolve, reject) => {
    ...
    req.end();
      ...
      /*  `https://usher.ttvnw.net/${
        ...
        }player=twitchweb&allow_audio_only=true&allow_source=true&type=any&p=${Math.floor(
      Math.random() * 10000
    )}*/
function getPlaylist(id, accessToken, vod) {
  return new Promise((resolve, reject) => {
    const req = https
      .get(`http://usher.ttvnw.net/api/channel/hls/${id}.m3u8?player=twitchweb&&token=${encodeURIComponent(
      accessToken.value
    )}&sig=${accessToken.signature}&allow_audio_only=true&allow_source=true&type=any&p=${Math.floor(
      Math.random() * 10000
    )}`
,
        (response) => {
